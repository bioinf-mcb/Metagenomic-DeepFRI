...
def parse_args():
    ...
    parser.add_argument("-o", "--output", required=False, default=FOLDSEEK_DATABASES_PATH)
...
def update_atoms_dataset(input_path, atoms_path, output_path, tmp_path, overwrite):
    ...
    print("Creating new foldseek database in " + str(db_path))
    run_command(f"foldseek createdb {STRUCTURE_FILES_PATH} {db_path / TARGET_DB_NAME}")

    print("Indexing new foldseek database in " + str(db_path))
    run_command(f"foldseek createindex {db_path / TARGET_DB_NAME} {tmp_path}")
